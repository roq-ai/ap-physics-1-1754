...
import { getQuestionGenerations } from 'apiSdk/question-generations';
import { QuestionGenerationInterface } from 'interfaces/question-generation';

import { QuestionListPage as ShowCaseList } from 'pages/questions';

function DashboardPage() {
  const { session } = useSession();

  const questionStat = useSWR<PaginatedInterface<QuestionInterface>>(
    () => `/questions`,
    () => getQuestions({ limit: 1, offset: 0 }),
  );

  const topicStat = useSWR<PaginatedInterface<TopicInterface>>(
    () => `/topics`,
    () => getTopics({ limit: 1, offset: 0 }),
  );

  const questionGenerationStat = useSWR<PaginatedInterface<QuestionGenerationInterface>>(
    () => `/question-generations`,
    () => getQuestionGenerations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Questions',
      result: questionStat?.data?.totalCount,
      isLoading: questionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Topics',
      result: topicStat?.data?.totalCount,
      isLoading: topicStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Question Generations',
      result: questionGenerationStat?.data?.totalCount,
      isLoading: questionGenerationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`questions`} passHref legacyBehavior>
                  <Button
