...
type ColumnType = ColumnDef<QuestionInterface, unknown>;
...
export function QuestionListPage(props: QuestionListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getQuestions({
        ...
        order: params.order,
        searchTermKeys: ['content.contains', 'question_topic.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<QuestionInterface>>(
    () => `/questions?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: QuestionInterface) => {
    if (hasAccess('question', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/questions/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'content', header: 'Content', accessorKey: 'content' },
    { id: 'question_topic', header: 'Question Topic', accessorKey: 'question_topic' },
    hasAccess('template', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/templates/view/${record.template?.id}`}>
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/questions/view/${record.id}`} passHref legacyBehavior>
                <Button
